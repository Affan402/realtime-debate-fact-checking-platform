...
export default function ArgumentsPage() {
  ...
  const arguments_ = rawArgs.map((arg: any) => {
    const score: number = arg.credibilityScore ?? 0
    const strength: "strong" | "medium" | "weak" = score >= 0.7 ? "strong" : score >= 0.4 ? "medium" : "weak"
    const issueType: "verified" | "disputed" | "low-credibility" = score >= 0.7 ? "verified" : score >= 0.4 ? "disputed" : "low-credibility"
    const hasFallacy = Boolean(arg.fallacy && arg.fallacy !== "None" && !(typeof arg.fallacy === "object" && Object.keys(arg.fallacy).length === 0))
    return {
      id: arg.id,
      speaker: {
      ...
      reactions: 0,
      strength,
      hasWarning: hasFallacy,
      warningType: (hasFallacy ? "fallacy" : undefined) as "fallacy" | undefined,
      factCheckStatus: {
        isChecking: false,
        hasIssue: score < 0.7,
        issueType,
